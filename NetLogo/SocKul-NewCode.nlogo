;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Setup
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

extensions [ nw ]

globals [  ;; global variables
  mean-path-length ;; average path length between the nodes
  adopter-size-list
  active-agents
  network-density
  avg-clustering-coef
  avg-degree
  max-deg
  min-deg
  modularity
  network-diameter
  seed-nodes
]

turtles-own [
  adopt?  ;; whether or not the agent has been infected with the malicious content
  clustering-coefficient ; defines the likelihood that two randomly selected neighbors of a node are neighbors themselves
  has-spread?  ;; whether or not the agent has tried to infect neighbors
  degree ;; degree of the node
  influence  ;; influence measure calculated from ks-degree
  dis-degree ; discount degree
  ks-degree ; k-shell degree
  nb-seed-neighbors ; number of neighbors of the turtle that are already selected as seeds
  eigenvector-centrality
]

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Clear functions
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

to setup
  clear-all
  reset-ticks
  set active-agents no-turtles  ;; Initialize active-agents as an empty turtle set
  set seed-nodes no-turtles  ;; Initialize seed-nodes as an empty turtle set
  create-network
end


;; Create the social network
to create-network
  ;; Generate network based on chosen type
  generate-network

  ;; Calculate network measures
  calculate-network-measures

  ;; set initial adopters
  spread-seed-nodes
end

to generate-network
  if network-type = "Barabasi-Albert" [
    nw:generate-preferential-attachment turtles links number_nodes attachment [
      initialize-turtle
    ]
  ]
  if network-type = "Erdos-Renyi" [
    nw:generate-random turtles links number_nodes attachment / number_nodes [
      initialize-turtle
    ]
  ]
  if network-type = "Small-World" [
    nw:generate-watts-strogatz turtles links number_nodes neighborhood-size rewiring-probability [
      initialize-turtle
    ]
  ]
end

to spread-seed-nodes
  if seeding-method = "random" [
    ask n-of nb-seed-nodes turtles [
      set adopt? true
      set has-spread? true
      update-color
    ]
  ]
  if seeding-method = "betweenness" [
    ask max-n-of nb-seed-nodes turtles [ nw:betweenness-centrality ] [
      set adopt? true
      set has-spread? true
      update-color
    ]
  ]
  if seeding-method = "eigenvector" [
    ask max-n-of nb-seed-nodes turtles [ eigenvector-centrality ] [
      set adopt? true
      set has-spread? true
      update-color
    ]
  ]
  if seeding-method = "closeness" [
    ask max-n-of nb-seed-nodes turtles [ nw:closeness-centrality ] [
      set adopt? true
      set has-spread? true
      update-color
    ]
  ]
  if seeding-method = "degree-discount" [
    degree-discount
    ask seed-nodes [
      set adopt? true
      set has-spread? true
      update-color
    ]
  ]
  if seeding-method = "k-shell" [
    k-shell
    ask seed-nodes [
      set adopt? true
      set has-spread? true
      update-color
    ]
  ]

  set active-agents (turtle-set active-agents turtles with [adopt?])
end



to degree-discount ; degree discount heuristics developed by Wei Chen et al.
  ask turtles [
    set dis-degree count link-neighbors ;initiate the discount degree for all the nodes
    set nb-seed-neighbors count link-neighbors with [member? self seed-nodes]
  ]
  let node-counter 0
  while [node-counter < nb-seed-nodes] ; node-counter means number of seed nodes
  [
    let u max-one-of turtles with [not member? self seed-nodes][dis-degree]
    set seed-nodes (turtle-set u seed-nodes)
    ask u [
      ask link-neighbors with [not member? self seed-nodes]
      [
        set nb-seed-neighbors nb-seed-neighbors + 1
        set dis-degree count link-neighbors - 2 * nb-seed-neighbors - (count link-neighbors - nb-seed-neighbors) * diffusion-probability * nb-seed-neighbors
      ]
    ]
    set node-counter node-counter + 1
  ]
end


to k-shell
 let my-count 1
 set seed-nodes no-turtles  ;; ensure the seed-nodes are empty before starting
 ask turtles [
   set ks-degree 0
   set influence 0
 ]
 while [any? turtles with [ks-degree = 0]]
 [
   while [any? turtles with [ks-degree = 0 and (count link-neighbors with [ks-degree = 0]) <= my-count]]
   [
     ask turtles with [ks-degree = 0 and (count link-neighbors with [ks-degree = 0]) <= my-count]
     [
       set ks-degree my-count * 5000
       set influence ks-degree + count link-neighbors
     ]
   ]
   set my-count my-count + 1
 ]
 set seed-nodes max-n-of nb-seed-nodes turtles [influence]
end


to initialize-turtle
  set size 2
  set shape "circle"
  set color blue
  set adopt? false
  set has-spread? false
end

to calculate-network-measures
  ;; Calculate degree, clustering coefficient and eigenvector centrality for each node
  nw:set-context turtles links
  ask turtles [
    set degree count link-neighbors
    set clustering-coefficient nw:clustering-coefficient
    set eigenvector-centrality nw:eigenvector-centrality
  ]

  ;; Calculate and display network density
  set network-density (2 * count links) / (count turtles * (count turtles - 1))

  ;; calculate and display average clustering coefficient
  set avg-clustering-coef mean [clustering-coefficient] of turtles

  ;; calculate and display average node degree
  set avg-degree sum([count link-neighbors] of turtles) / count turtles

  ;; calculate max node degree
  set max-deg max [count link-neighbors] of turtles

  ;; calculate min node degree
  set min-deg min [count link-neighbors] of turtles

  ;; calculate the modularity
  set modularity nw:modularity nw:louvain-communities

  ;; calculate the mean path length
  let path-lengths nw:mean-path-length
  set mean-path-length path-lengths
  ;; Calculate and display other network measures as per your original code...
end



to go
  let newly-adopted no-turtles
  ask active-agents [
    let target-neighbors ifelse-value (links-to-use = "outward") [out-link-neighbors] [link-neighbors]
    ask target-neighbors [
      if not adopt? and random-float 1.0 < diffusion-probability [
        set adopt? true
        set newly-adopted (turtle-set newly-adopted self)
      ]
    ]
    set has-spread? true
  ]

  ;; After the infection attempt, clear the active-agents list
  set active-agents no-turtles

  ;; Newly infected agents become the active agents for the next tick
  set active-agents newly-adopted

  ;; Updating color and percentage of infected agents
  ask turtles [ update-color ]

  calculate-adopter-size
  tick


  if not any? active-agents [
    print "There are no more attempts at infections"
    stop
  ]
end


to calculate-adopter-size ;; Added a procedure to calculate the percentage of infected turtles
  set adopter-size-list (count turtles with [adopt?]) / (count turtles) * 100
end

;; utility procedures
to update-color
  if adopt? [
    set color red
    set shape "target"
  ]
end

to layout
   layout-spring turtles links 1 25 3.0
  display
end

to community-detection
  nw:set-context turtles links
  color-clusters nw:louvain-communities
end

to color-clusters [ clusters ]
  let n length clusters
  let main-colors [ blue green red ]
  let colors n-values n [ item (random length main-colors) main-colors + random-float 2 - 1 ]

  (foreach clusters colors [ [cluster cluster-color] ->
    ask cluster [ ; for each node in the cluster
      ifelse adopt? [
        set color red ; set infected node color to red
      ] [
        set color cluster-color ; give the node the color of its cluster if it is not sensitive or infected
      ]
      ; colorize the links from the node to other nodes in the same cluster
      ; link color is slightly darker...
      ask my-links [ if member? other-end cluster [ set color cluster-color - 1 ] ]
    ]
  ])
end
@#$#@#$#@
GRAPHICS-WINDOW
379
10
1003
635
-1
-1
6.1
1
10
1
1
1
0
1
1
1
-50
50
-50
50
1
1
1
ticks
30.0

BUTTON
288
562
374
595
layout
layout
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
1
562
289
595
setup
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
1
596
375
629
go
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

CHOOSER
0
174
179
219
seeding-method
seeding-method
"betweenness" "random" "eigenvector" "closeness" "k-shell" "degree-discount"
4

CHOOSER
0
41
376
86
network-type
network-type
"Barabasi-Albert" "Erdos-Renyi" "Small-World"
1

SLIDER
0
126
181
159
diffusion-probability
diffusion-probability
0
1
0.3
0.05
1
NIL
HORIZONTAL

SLIDER
0
89
180
122
number_nodes
number_nodes
5
250
250.0
5
1
NIL
HORIZONTAL

SLIDER
185
89
376
122
nb-seed-nodes
nb-seed-nodes
0
10
9.0
1
1
NIL
HORIZONTAL

SLIDER
0
324
172
357
attachment
attachment
1
10
2.0
1
1
NIL
HORIZONTAL

PLOT
1010
452
1310
634
Degree Distribution
Degree
# of nodes
2.0
10.0
0.0
10.0
true
false
"clear-plot" "clear-plot\n"
PENS
"default" 1.0 1 -16777216 false "let max-degree max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [count link-neighbors] of turtles" "let max-degree max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [count link-neighbors] of turtles"

MONITOR
1010
311
1098
356
#links
count links
1
1
11

MONITOR
1105
360
1202
405
max-deg
max-deg
1
1
11

MONITOR
1207
360
1308
405
min-deg
min-deg
1
1
11

MONITOR
1011
359
1099
404
avg-degree
avg-degree
3
1
11

TEXTBOX
1116
238
1266
257
Graph Metrics
15
0.0
1

PLOT
1010
10
1309
217
Spread of adoption
Ticks
Adopted %
0.0
10.0
0.0
100.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot adopter-size-list"

MONITOR
1010
265
1152
310
mean-path-length
mean-path-length
1
1
11

MONITOR
1104
311
1201
356
avg-clustering-coef
avg-clustering-coef
3
1
11

MONITOR
1207
312
1308
357
network-density
network-density
3
1
11

SLIDER
205
325
380
358
neighborhood-size
neighborhood-size
0
floor ((number_nodes - 1) / 2)
4.0
1
1
NIL
HORIZONTAL

SLIDER
205
359
380
392
rewiring-probability
rewiring-probability
0
1
0.1
0.01
1
NIL
HORIZONTAL

TEXTBOX
106
10
256
31
Generel parameters
17
0.0
1

TEXTBOX
84
267
309
309
Network-specific Parameters
17
0.0
1

TEXTBOX
33
315
183
333
NIL
11
0.0
1

TEXTBOX
38
304
188
322
Barabasi-Albert
12
0.0
1

TEXTBOX
279
324
429
342
NIL
11
0.0
1

TEXTBOX
258
304
408
322
Small-world
12
0.0
1

BUTTON
226
526
373
559
NIL
community-detection
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SWITCH
0
526
171
559
community-highlight?
community-highlight?
0
1
-1000

CHOOSER
183
173
374
218
links-to-use
links-to-use
"undirected" "outward"
0

@#$#@#$#@
## WHAT IS IT?


## HOW IT WORKS


## HOW TO USE IT












Please cite the NetLogo software as:

* Wilensky, U. (1999). NetLogo. http://ccl.northwestern.edu/netlogo/. Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

sheep
false
15
Circle -1 true true 203 65 88
Circle -1 true true 70 65 162
Circle -1 true true 150 105 120
Polygon -7500403 true false 218 120 240 165 255 165 278 120
Circle -7500403 true false 214 72 67
Rectangle -1 true true 164 223 179 298
Polygon -1 true true 45 285 30 285 30 240 15 195 45 210
Circle -1 true true 3 83 150
Rectangle -1 true true 65 221 80 296
Polygon -1 true true 195 285 210 285 210 240 240 210 195 210
Polygon -7500403 true false 276 85 285 105 302 99 294 83
Polygon -7500403 true false 219 85 210 105 193 99 201 83

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

wolf
false
0
Polygon -16777216 true false 253 133 245 131 245 133
Polygon -7500403 true true 2 194 13 197 30 191 38 193 38 205 20 226 20 257 27 265 38 266 40 260 31 253 31 230 60 206 68 198 75 209 66 228 65 243 82 261 84 268 100 267 103 261 77 239 79 231 100 207 98 196 119 201 143 202 160 195 166 210 172 213 173 238 167 251 160 248 154 265 169 264 178 247 186 240 198 260 200 271 217 271 219 262 207 258 195 230 192 198 210 184 227 164 242 144 259 145 284 151 277 141 293 140 299 134 297 127 273 119 270 105
Polygon -7500403 true true -1 195 14 180 36 166 40 153 53 140 82 131 134 133 159 126 188 115 227 108 236 102 238 98 268 86 269 92 281 87 269 103 269 113

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.3.0
@#$#@#$#@
setup repeat 40 [ layout ] repeat 15 [ go ]
@#$#@#$#@
@#$#@#$#@
<experiments>
  <experiment name="varying-seed-nodes" repetitions="500" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>behaviorspace-run-number</metric>
    <metric>number_nodes</metric>
    <metric>count links</metric>
    <metric>avg-degree</metric>
    <metric>network-density</metric>
    <metric>modularity</metric>
    <metric>avg-clustering-coef</metric>
    <metric>mean-path-length</metric>
    <metric>adopter-size-list</metric>
    <metric>count turtles with [adopt?]</metric>
    <metric>max-deg</metric>
    <metric>min-deg</metric>
    <metric>attachment</metric>
    <metric>neighborhood-size</metric>
    <metric>rewiring-probability</metric>
    <enumeratedValueSet variable="network-type">
      <value value="&quot;Barabasi-Albert&quot;"/>
      <value value="&quot;Small-World&quot;"/>
      <value value="&quot;Erdos-Renyi&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="seeding-method">
      <value value="&quot;random&quot;"/>
      <value value="&quot;betweenness&quot;"/>
      <value value="&quot;eigenvector&quot;"/>
      <value value="&quot;closeness&quot;"/>
      <value value="&quot;degree-discount&quot;"/>
      <value value="&quot;k-shell&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="nb-seed-nodes" first="1" step="1" last="10"/>
    <steppedValueSet variable="diffusion-probability" first="0.1" step="0.1" last="0.3"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
0
@#$#@#$#@
